...
def graphcast_precip_forecast(start_time, lead_days=10, bbox=PUGET_CASCADES_BBOX,
                              model_name="GraphCastSmall", out=None):
    ...
    out_ds.attrs.update(source=f"{model_name}/earth2studio", native_res_deg=1.0,
                        start_time=str(start_time), lead_days=lead_days, dt_days=1.0,
                        caveat="1 deg grid: no orographic enhancement over the Cascades")
    if out:
...
def main():
    ...
    p.add_argument("--lead-days", type=int, default=10)
    p.add_argument("--model", default="GraphCastSmall", choices=sorted(PRECIP_CAPABLE))
    ...
    a = p.parse_args()
    graphcast_precip_forecast(a.start, a.lead_days, tuple(a.bbox), a.model, a.out)
