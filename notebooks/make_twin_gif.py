"""Digital-twin GIF: the soil state evolving through the 2025-26 wet season (western Cascades).
...
STEP = 8              # spatial subsample (keeps the GIF a few MB, not ~70)
CADENCE_DAYS = 7      # weekly frames
...
def main():
    ...
    hd = _tif("terrain_hand_domain_90m.tif").values[sl]
    tb = np.tan(np.radians(_tif("terrain_slope_domain_90m.tif").values[sl]))
    v0 = _tif("vs30_domain_90m.tif").values[sl]
    rd = soil.root_depth_m.values[sl]

    fo = ForecastForcing(times=times.values, precip_mm=PR, pet_mm=PE, dt_days=1.0,
                         tmean_c=TM, source="PRISM")
    ...
    idx = np.arange(0, len(times), CADENCE_DAYS)
    TH = np.stack([msk(fx.theta[i]) for i in idx])
    WTR = np.stack([msk(d0 - fx.wt_depth_m[i]) for i in idx])          # +ve = table risen
    ...
    series = [(TH, "Soil moisture θ", "YlGnBu", "m³ m⁻³"),
              (WTR, "Water table rise", "Blues", "m above baseline"),
              (DV, "dv/v  (shallow band)", "RdBu", "%")]
    ims = []
    for a, (D, t, cmap, unit) in zip(ax[:3], series):
        v = D[np.isfinite(D)]
        ...
        cm.set_bad("#eef0f3")
        im = a.imshow(D[0], cmap=cm, vmin=lo, vmax=hi)
        ims.append(im)
    ...
    def upd(k):
        for im, (D, _, _, _) in zip(ims, series):
            im.set_data(D[k])
    ...
    FuncAnimation(fig, upd, frames=len(idx), blit=False).save(
