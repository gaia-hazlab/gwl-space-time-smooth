...
The r&nbsp;=&nbsp;0.98 agreement with TerraClimate above is a *consistency check* — shared
...
The result is a sobering, useful one: **pooled r&nbsp;=&nbsp;0.35**
...
— e.g. Park Creek Ridge r&nbsp;=&nbsp;0.66&nbsp;→&nbsp;0.78 — while the
*pooled* r stays bias-limited (0.33&nbsp;→&nbsp;0.35):
...
```{=html}
<div class="grid">
  <div class="stat"><div class="n accent">0.35</div><div class="l">independent θ validation (SNOTEL in-situ, uplands) — vs 0.98 shared-forcing consistency</div></div>
  <div class="stat"><div class="n sm">0.66→0.78</div><div class="l">best-site seasonal r, snow module off → on (Park Creek Ridge)</div></div>
</div>
```
...
operational anchor that **consumes SNOTEL as training**, leaving SMAP ([#29](https://github.com/gaia-hazlab/gwl-space-time-smooth/issues/29)) the
independent test — collapses RMSE 0.107&nbsp;→&nbsp;0.070
...
from -0.045 to -0.010 m³/m³ (a
4× reduction). RMSE is essentially
flat (0.102→0.108) — with only
5 sparse upland stations, one alpine outlier's bias can't be resolved
...
r&nbsp;=&nbsp;0.81** (per-cell r&nbsp;=&nbsp;0.79) over
2024-01–2025-12 — the seasonal cycle and its 2024/2025 interannual swing
line up closely. The residual is the familiar dry **bias** (-0.11 m³/m³): the bucket
...
2024–2025: the seasonal phase and interannual swing match (r≈0.81);
the offset is the field-capacity dry bias.](../figures/demo/merra2_validation.png)

```{=html}
<div class="grid">
  <div class="stat"><div class="n sm">0.81</div><div class="l">vs MERRA-2 root-zone reanalysis (depth-matched, native 0.5°, incl. 2025)</div></div>
...
- **The r&nbsp;=&nbsp;0.98 agreement is a consistency check, not independent validation** — our bucket and
  ...
  The independent check is the SNOTEL comparison above (pooled r≈0.35); SMAP is next ([#29](https://github.com/gaia-hazlab/gwl-space-time-smooth/issues/29)).
