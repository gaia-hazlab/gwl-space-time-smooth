...
$r = 0.98$ shared-forcing consistency, which is exactly the point. The check also localised the
...
The pieces above compose into a **digital twin**: a coupled, uncertainty-aware, 90 m estimate of
...
the matching per-cell 1σ. The static structure is real 90 m data; the time evolution and its
high-frequency content come from assimilating the depth-separated dv/v onto those fields, so the
animation literally shows the assimilation at work — the state updates and σ shrinks where and when
...
| NRCS SNOTEL (SMS) | NRCS AWDB REST API, element SMS | point | soil-moisture assimilation |
...
| Vs30 (Sanger-Maurer 2025) | GAIA vs-STAC (`fetch_gaia`); HAND fallback | 90 m | Vs30 static base |
...
| SMAP L3 (SPL3SMP_E) | NASA NSIDC (`earthaccess`) | 9 km | soil moisture (independent, staged) |
| SNOTEL θ (held out) | NRCS AWDB | point | soil-moisture phase (LOSO) |
| NWIS wells (held out) | USGS | point | GWL baseline (spatial block-CV) |

### Attribution: which data streams set each state

Because the assimilation is precision-weighted, each observation's contribution to a cell is exactly
its share of the total precision, $w_i = \exp(-d^2/2L^2)/\sigma_i^2$; summing per source and adding
the model-prior precision $p_0 = 1/\sigma_{\text{prior}}^2$ gives an **exact attribution** of every
estimate to its data streams (and to the model). @fig-attr maps the dominant stream and reports the
domain-mean shares: groundwater is set ~94% by the dense well network, soil moisture ~95% by the
seismic dv/v (which has more stations than the sparse SNOTEL soil-moisture set), and Vs30 ~99% by
the seismic dv/v. The maps show *where* each stream wins and where the estimate reverts to the model
prior between sensors — a direct, per-pixel accounting of what the twin is actually listening to.

![Data-stream attribution. Top: the dominant data stream at each cell (wells, SNOTEL, seismic dv/v,
or model prior). Bottom: domain-mean attribution share per state. Computed exactly from the
assimilation weights.](assets/digital_twin_attribution.png){#fig-attr}
...
|---|---|---|---|
| Groundwater baseline | block-CV RMSE | ≈ 18.5 m | held-out wells |
| Soil moisture | domain-mean $r$; bias; RMSE | 0.85; −0.08; 0.095 | MERRA-2 root zone (incl. 2025) |
| Soil moisture (snow phase) | LOSO $r$ | 0.49 (tuning overfits to 0.44) | SNOTEL |
| dv/v measurement | band recovery $r$ | > 0.95 | controlled synthetic |
| Assimilation | posterior σ reduction | WTD ~53%, θ ~91% | near-station vs prior |
...
(4) carry the per-month kriging σ rather than its time mean; (5) fetch the GAIA Vs30 product in
place of the HAND proxy; (6) densify the Cascade networks so the anchor corrects station-specific
representativeness, not only the systematic bias.

### Toward forecast forcing: Earth2Studio {#sec-earth2studio}

The MVP is a *hindcast* digital twin. The same machinery becomes a *forecast* twin by swapping the
forcing: AI weather models (GraphCast, AIFS, StormCast, served through **Earth2Studio**) provide
forecast precipitation and reference ET, which drive the same soil-moisture bucket and the Stage-2
β-map / TFN reservoir to produce forecast groundwater, soil moisture, and Vs30 — as ensembles that
carry the forecast spread into the state uncertainty. Because Stage 2 is a linear operator on
climate indices, a new scenario is a matrix multiply, not a re-solve, so a 50-member forecast
ensemble is cheap. This is deliberately staged after the MVP: we first prove the coupled,
uncertainty-aware hindcast with in-situ assimilation, then bring numerical-weather-prediction
forcing to turn the twin forward in time.
