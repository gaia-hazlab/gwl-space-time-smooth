...
The two grounds meet in petrophysics: water changes stiffness, and the ambient-noise velocity change
$\delta v/v$ measures it [@denolle2026unified]. Two regimes, split at the water table, carry two
different hydrologic signals.

**Saturated zone — pore pressure.** Below the water table a change in head changes effective stress
and thus velocity, the poroelastic response of Biot's theory of a fluid-saturated porous solid
[@biot1941general]. To first order the module writes it as a linear relation $\delta v/v = k_{sat}\,\Delta h$,
so a *relative* water-table change is recovered (a rising table — smaller depth-to-water — lowers
effective stress and softens the medium). This is the mechanism by which the ambient field tracks
groundwater: seasonal velocity variations follow the water table at volcanoes and aquifers alike
[@senss2006passive], and @clements2018tracking recover GWL time series directly from $\delta v/v$.
Consistent with that literature, $\delta v/v$ constrains a relative, not an absolute, level.

**Vadose zone — partial saturation and suction.** Above the water table stiffness is governed by
grain-contact mechanics (Hertz–Mindlin) modulated by capillary **suction**: as the soil dries,
matric suction pulls grains together and stiffens the frame, so wetting *softens* it. The module
writes $\delta v/v = S_\theta\,\Delta\theta$ with $S_\theta < 0$. The material sensitivity is order
$S_\theta \approx -1$ per unit $\theta$ (a $\sim$10 % velocity change per 0.1 m³ m⁻³ locally), but a
raw seismic band averages this fine near-surface response over its whole depth kernel; only the top
$\sim$1 m of the kernel is $\theta$-active, so a single band shows the depth-diluted
$S_{\theta,\text{band}} \approx -0.08$ that reproduces the observed 0.1–1 % seasonal band $\delta v/v$.
The depth inversion deconvolves this dilution and returns the material value, which is why the module
applies $S_\theta = -1$ to the *inverted* profile and $S_{\theta,\text{band}}$ only to a raw band.

**The linking equations.** The frequency→depth map and the two regime laws, split at the water table
depth $z_{wt}$, are

$$
L(f) \approx \frac{V_S}{3f}, \qquad
V_{S30} = \frac{30}{\displaystyle\int_0^{30}\! dz/V_S(z)}, \qquad
\frac{\delta v}{v}(z) =
\begin{cases}
S_\theta\,\Delta\theta(z), & z < z_{wt} \ \text{(vadose: suction, Hertz–Mindlin)},\\[4pt]
k_{sat}\,\Delta h, & z > z_{wt} \ \text{(saturated: poroelastic, Biot)}.
\end{cases}
$$

The banded measurements are inverted for the depth profile through the Rayleigh sensitivity kernels
$\mathbf{G}$, $\ \mathbf{d} = \mathbf{G}\,\mathbf{m} + \boldsymbol{\epsilon}$, with the data covariance
of $\boldsymbol{\epsilon}$ marginalised over the processing ensemble ([`codameter`](https://github.com/Denolle-Lab/codameter)).
Splitting the resulting profile at $z_{wt}$ yields two hydrologic states from one seismic measurement —
a shallow **soil-moisture** change (vadose, suction) and a deep **relative water-table** change
(saturated, poroelastic) — each assimilated into the matching hydrological state above.
