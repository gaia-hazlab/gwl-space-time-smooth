...
- **Vertical infiltration / recharge** $R$ — the deep-percolation flux when the root-zone store
  exceeds field capacity, feeding a linear-reservoir water table $h_{t} = h_{t-1} + R/S_y - h_{t-1}/\tau$
  with specific yield $S_y$ and recession timescale $\tau$ [@brutsaert1977recession];
- **Capillary rise** — an upward flux from a shallow water table back into the root zone across a
  capillary fringe, the return term that couples GWL to soil moisture;
- **Evapotranspiration** — the atmospheric demand removed from the store toward wilting;
- **Saturation-excess surface runoff** — generated where the store is at capacity;
- **Subsurface lateral flow** — a TOPMODEL redistribution [@beven1979physically] setting each cell's
  water-table deficit from its topographic wetness index, $d_i = \bar d - m\,(\mathrm{TWI}_i - \overline{\mathrm{TWI}})$.

The scheme conserves mass by construction — the change in storage equals capillary rise plus liquid
input minus runoff, ET, and recharge — so the vertical exchange between soil moisture and the water
table, and the downhill routing of excess water, are represented rather than assumed away. This is the
sense in which the two water states are *coupled* and not merely co-located.
...
Splitting the depth-separated $\delta v/v$ profile at the water table therefore yields two hydrologic
states from one seismic measurement — a shallow **soil-moisture** change (vadose, suction) and a deep
**relative water-table** change (saturated, poroelastic) — each assimilated into the matching
hydrological state above.
