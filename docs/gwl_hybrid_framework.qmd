...
TerraClimate) gives a **pooled $r \approx 0.33$** against the measured $\theta$ — with the best
sites reaching $r \approx 0.67$ on the seasonal phase but a systematic dry bias at the highest,
snowiest stations. That the independent number is far below the $r = 0.98$ shared-forcing
consistency is exactly the point: these are snowmelt-driven uplands and the bucket has no
explicit snowpack, so the check both bounds the honest skill and localises the missing physics.
Further independent validation (SMAP retrievals, denser in-situ probes) remains future work.
