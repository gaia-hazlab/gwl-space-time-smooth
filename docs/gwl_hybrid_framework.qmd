...
mean, so `seasonal_high_water_table` exports the shallow-tail quantile of depth-to-water alongside
the mean. Every field carries its per-cell 1σ and the variogram-driven confidence mask (@sec-validation):
LandLab ingests uncertainty, and masked upland cells (fractured hillslopes — precisely the sparse-well
regime where this framework is weakest) are exported as no-data rather than a false mean.
