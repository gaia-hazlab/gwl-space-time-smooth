...
its permutation importance is the honest, field-wide accounting: **Vs30 is ~100% terrain (HAND)**,
groundwater is ~61% HAND and ~26% clay, and soil moisture is ~89% sand. The dv/v is a small
field-level contributor here **by construction**: in this MVP it only perturbs the state near the
seismic stations, so it cannot explain the covariate-driven structure that fills the domain. This is
