...
(bottom), on the 90 m grid, masked to the well-supported domain. The water table carries fine
terrain-driven (HAND) 90 m structure; the saturation-fraction and recharge fields inherit the coarse
~4 km TerraClimate driver footprint (visible as blocky cells) — an honest signal of their native
resolution, recorded in the export manifest so a resampled cell is not read as a native 90 m
observation.](assets/landlab_export.png){#fig-landlab-export}
