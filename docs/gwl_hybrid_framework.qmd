...
the matching per-cell 1σ. The groundwater and soil-moisture static structure is real 90 m data; the
**Vs30 shown here is a synthetic HAND-ramp placeholder** (see the classification table), not a
measured or published Vs30. The time evolution and its high-frequency content come from a **synthetic**
...
| Vs30 (synthetic HAND ramp in the MVP) | GAIA vs-STAC (`fetch_gaia`) not reached; **HAND fallback used** | 90 m | Vs30 static base (placeholder — see classification table) |
...
| Vs30 | **synthetic** (HAND ramp) | no | Sanger-Maurer parametric (GAIA vs-STAC); Wald-Allen slope proxy; measured Vs30 (MASW / borehole / Vs30 databases) |
...
**The water budget (required, not yet closed).** The current bucket removes above-field-capacity water
as drainage that simply *leaves the system*; groundwater and soil moisture are therefore co-located,
not hydrologically coupled. A correct coupled reanalysis must route that water through two fluxes:

1. **Vertical infiltration / recharge.** Deep percolation below the root zone must recharge the water
   table (an explicit theta -> GWL flux), with capillary rise back into the vadose zone where the
   table is shallow. This is what makes "coupled" physical rather than nominal.
2. **Downhill runoff.** Surface runoff (infiltration- and saturation-excess overland flow) and
   subsurface lateral flow (interflow / flow along the water table) must redistribute water
   down-gradient, so the column is not a closed 1-D bucket.

Until both fluxes are represented, the product is an *antecedent-state estimate*, not a closed water
balance, and cannot by itself drive a runoff or flood model. Closing the water budget is tracked as a
priority modeling task (see the project issues).
...
its permutation importance is the field-wide accounting: **Vs30 is ~100% terrain (HAND)**, groundwater
is ~61% HAND and ~26% clay, and soil moisture is ~89% sand. **Caveat (partly tautological in this
MVP).** Because the synthetic Vs30 is *defined* as a function of HAND and the bucket θ is *defined*
from Saxton-Rawls texture (sand/clay), the random forest recovers those generative covariates by
construction — the ~100% HAND and ~89% sand are self-recovery, not a discovery about real soil. The
attribution becomes a genuine finding only once Vs30 and θ are real fields whose covariate relations
are learned, not imposed. The dv/v is a small field-level contributor here also **by construction**:
...
| Assimilation | posterior σ reduction | WTD ~53%, θ ~91% | near-station vs prior | **SYNTHETIC** (geometry only; zero measurement error injected) |

The **Basis** column is essential: only the groundwater block-CV RMSE and the SNOTEL LOSO are real
independent skill. MERRA-2 is a model cross-check (shared forcing family). The dv/v recovery and the
assimilation σ-reduction are synthetic-derived — the recovery is circular (the inverse undoing the
forward operator on an imposed signal), and the σ-reduction reflects station geometry with **zero
dv/v measurement error injected**, not demonstrated dv/v precision. These are pipeline demonstrations,
not skill claims, until the real dv/v run lands.
