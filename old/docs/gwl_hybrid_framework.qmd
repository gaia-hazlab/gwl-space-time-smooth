---
title: |
  The GAIA Digital Twin of Soil: Observation-Anchored Groundwater Level, Soil Moisture,
...
date-format: long
abstract: |
  The GAIA Digital Twin of Soil — a coupled soil reanalysis as its end product — maintains a *living
  hydromechanical state of the subsurface* —
  water table depth, soil moisture, and near-surface stiffness — at the space and time
  scales that downstream hazard models (flood, landslide, liquefaction) require. The state is
  constrained by high-resolution static layers, ground sensors, and satellite retrievals, tied
  together by a closed water budget and a hydromechanical rheology, and scored against held-out
  validation at native sensor resolution (@sec-mission). This report documents the first mature
  state variable and the shared machinery.
  ...
  well record. We are explicit about what is demonstrated here versus what is built but
  untested: the real-data 90 m monthly product reported below obtains its Stage-2/3
  dynamic term by ordinary-kriging the *observed* per-well anomalies onto the static
  baseline; the β-map path is implemented in the pipeline but has not yet been fit or
  validated on the real well network, and the transfer-function-noise forecast model
  discussed as its successor is not implemented at all. The framework is *designed* to
  represent four PNW hydroclimatic signals: seasonal storm recharge, late-season snowmelt,
  atmospheric river events, and multi-year ENSO/PDO modulation. It does not resolve them as
  separate terms today — that attribution lives in the unfit β-map path, whose
  atmospheric-river term is identically zero. Outputs are formatted as GAIA HazLab xarray
  DataTree archives with STAC provenance metadata, suitable for direct ingestion by Sanger
  et al. geotechnical liquefaction models and LandLab geomorphological frameworks. We review prior hybrid
  ...
  precipitation-driven Thornthwaite–Mather water balance) and outline a third,
...
## Mission and Framework {#sec-mission}

**Mission.** Maintain a *living hydromechanical state of the subsurface* — water table depth,
soil moisture, and near-surface stiffness — at the space and time scales that downstream hazard
models need to anticipate **floods, landslides, and liquefaction**.

The one constraint that governs every design choice in this report: **the target spatial scale is
set by the hazard, not by whatever data happen to be available.** Liquefaction in valley fill,
shallow landslides on hillslopes, and compound flooding all resolve at ~100 m or finer, so the
state lives on a **90 m grid from the start** and every input is placed onto that grid at its own
native resolution, update rate, and uncertainty.
...
def _arrow(x0, y0, x1, y1, label=None, color=C_GREY, ls="-", lw=1.7):
    ax.add_patch(FancyArrowPatch((x0, y0), (x1, y1), arrowstyle="-|>",
        mutation_scale=14, linewidth=lw, color=color, linestyle=ls, zorder=1,
        shrinkA=1, shrinkB=1))
    if label:
...
1. **Static layers** — lithology, geology, soils, and terrain-derived hydrology. High-resolution
   but time-invariant; they set what the subsurface *can* do (the envelope of hydraulic and
   mechanical properties) and form the model prior.
2. **Ground-based sensors** — hydrologic (wells), geophysical (ambient-noise seismic), and
   atmospheric networks. Sparse in space, dense in time; they sense the state directly or by proxy /
   repurposed measurement (e.g. seismic dv/v as a moisture and pore-pressure proxy).
3. **Satellite products** — reprocessed retrievals (soil moisture, terrain, total water storage).
   Dense in space, sparse in time; their native resolution caps the resolution of anything derived
   from them.
4. **Validation data** — held out of processing and scored at the native resolution of the sensor,
   never upsampled to flatter the model.
5. **Physical models we constrain** — a closed **water budget** (rain in; runoff and ET out;
   infiltration to the water table and capillary rise back up) and a **hydromechanical state** that
   links moisture and pore pressure to stiffness through rheology and petrophysics.

**The assimilation principle.** Only the ground sensors (2) and satellites (3) are *assimilated* —
everything else is prior knowledge or physical modelling. Data enter at their own rates,
sensitivities, and uncertainties; the physical models are updated on a regular cadence; and the
output states — water table depth, soil moisture, Vs — are validated or benchmarked against the
held-out references (4), always at native sensor resolution.

The rest of this report is one concrete instantiation of this framework: groundwater level is the
mature state variable (@sec-framework), soil moisture is implemented (@sec-soilmoisture), the
seismically-constrained stiffness channel is in development (@sec-twin), and the coupling water
budget is specified in @sec-classification.
...
and the addition of a **β-coefficient map** framework [@bloomfield2013analysis] designed
to enable GWL forecasting from climate indices alone via a single matrix multiply —
without re-running any kriging step. That forecasting property is a design capability of
the implemented β-map code rather than a demonstrated result: it has not yet been fit or
validated on the real well network, and the real-data product presented in @sec-results
takes its time variation from kriged observed well anomalies instead.
...
## Scientific Grounds {#sec-grounds}

This reanalysis rests on three scientific grounds and the data beneath them. The **hydrological**
ground models the water state — groundwater level (GWL) and soil moisture; the **mechanical** ground
models the near-surface stiffness state — shear-wave velocity $V_S$ and density; and a set of
**petrophysical** models links the two, so a change in water produces a measurable change in
stiffness. The three map onto the gaia-hazlab science pillars: Pillar-1 soil reanalysis (hydrology),
the geotechnical hazard chain (mechanics — liquefaction and landslides), and the ambient-noise
$\delta v/v$ channel (the petrophysical bridge). We take each in turn, then the data grounds
(@sec-data-grounds) they all rest on.

### Hydrological modeling grounds {#sec-hydro-grounds}

Groundwater level and soil moisture are two faces of one subsurface water balance — the saturated
store below the water table and the vadose store above it — and both are modeled along the same
spectrum from process physics to pure statistics. We give them equal footing.
...
zone the analogue is the Richards equation, routinely reduced for regional work to a Thornthwaite–Mather
soil-water balance [@thornthwaite1957water]. These are physically consistent but demand aquifer
geometry and hydraulic properties poorly known at regional scale; @reinecke2020importance and
@degraaf2015global show that global models at 0.5°–10 km cannot resolve the water-table variability
...
[@beven1979physically], which converts a mean deficit into a spatial saturation pattern; recharge —
the vadose-to-saturated flux that couples the two stores — is itself a modeling choice with method-dependent
spread [@scanlon2002choosing].
...
@ma2026highres]. These share four limitations for our use: DEM-tile artifacts, elevation bias (valley
floors indistinguishable from ridge tops at equal elevation), no temporal variability, and severe
western-US extrapolation from sparse wells — HydroGEN still shows a +7.2 m bias / 20.5 m RMSE against
held-out PNW wells. The soil-moisture analogue is digital soil mapping: pedotransfer functions
[@saxton2006soil] applied to ML-mapped soil properties [@nauman2024solus; @ramcharan2018soil], and
regression kriging [@hengl2007regression], which separates a deterministic trend from kriged residuals.

**Data-driven / time-series.** At gauged sites, LSTM networks reach state-of-the-art one-step GWL
prediction [@wunsch2021groundwater; @liesch2019aquifer; @shen2018deeplearning] but do not generalize
to ungauged locations; transfer-function-noise models [@vonasmuth2002tfn; @bakker2019timeseries]
and the Pastas framework [@collenteur2019pastas] give parsimonious per-well impulse responses; and
climate-index regression links GWL to SPI, ENSO, and PDO [@bloomfield2013analysis; @kuss2014multi]
— the structure of our Stage-2 β-map. Random forests on terrain+geology regionalize groundwater
properties [@rodriguez2014nitrate; @marcais2017prospecting].

**Hybrid — and the gap.** True hybrids pairing a spatial ML/geostatistical baseline with a temporal
physics component are rare: process-model-augmented ML [@konapala2020hybrid; @heudorfer2019groundwater]
and GRACE-assimilating filters [@cuthbert2019global] are the closest precedents. No published product
combines HAND-based regression kriging, β-coefficient response maps, **and** a pixel-wise water budget
delivered with provenance for geotechnical/geomorphological consumers.

**Our ground: a pixel-wise water budget.** Rather than solve the flow equation or memorize wells, we
**enforce a closed water balance at every 90 m cell** (`src/models/water_budget.py`) and let
observations anchor it. Over the fine SOLUS→Saxton–Rawls storage envelope [@saxton2006soil], each
cell steps a monthly balance whose terms are all explicit:

- **Vertical infiltration / recharge** — deep percolation of the root-zone store above field capacity;
- **Capillary rise** — the upward return flux from a shallow water table into the root zone;
- **Evapotranspiration** — atmospheric demand removed from the store toward wilting;
- **Saturation-excess surface runoff** — generated where the store is at capacity;
- **Subsurface lateral flow** — a TOPMODEL redistribution of the water table by topographic wetness.

**The constrained equations.** The root-zone store $S=\theta\,z_r$ (mm over root depth $z_r$) is
stepped each month against the Saxton–Rawls limits $S_x=\theta_x z_r$ ($x\in\{wp,fc,sat\}$), enforcing
the closed balance ([`coupled_water_budget`](https://github.com/gaia-hazlab/gwl-space-time-smooth/blob/main/src/models/water_budget.py))

$$
S_t = S_{t-1} + C_t + I_t - Q_t - E_t - R_t, \qquad \theta_t = S_t/z_r,
$$

with the flux closures (mm month⁻¹; $[x]_+ \equiv \max(x,0)$ and $[x]_0^1 \equiv \min(\max(x,0),\,1)$ clamps to $[0,1]$)
...
with specific yield $S_y$, recession time $\tau$ [@brutsaert1977recession], capillary fringe $z_f$,
and the TOPMODEL slope $m$ [@beven1979physically]. This is the *integrated, per-pixel* counterpart of
the 3-D flow equation $S_s\,\partial h/\partial t=\nabla\!\cdot(K\nabla h)+R$ (@sec-hydro-grounds): we
do not solve the PDE, we **enforce its mass balance at every cell** — $\Delta S = C + I - Q - E - R$,
exact by construction — and let observations anchor the coefficients. The vertical exchange between
soil moisture and the water table (recharge $\leftrightarrow$ capillary rise) and the downhill routing
of excess water are therefore represented, not assumed away; this is the sense in which the two water
states are *coupled* rather than merely co-located.

### Mechanical modeling grounds {#sec-mech-grounds}

The mechanical state is the near-surface **shear-wave velocity** $V_S(z)$ and, through it, the small-strain
stiffness that geotechnical hazard models consume. It is constrained three ways, in decreasing data cost.
*Seismic tomography* from ambient noise inverts inter-station surface-wave dispersion for $V_S(z)$
[@shapiro2005high] — the highest-fidelity but sparsest constraint. *Vs30 proxies* — the 30 m
time-averaged velocity, $V_{S30} = 30 / \int_0^{30} dz/V_S(z)$ — trade fidelity for coverage: the
Wald–Allen topographic-slope model [@waldallen2007], which we adopt at native 90 m, and geology- and
geomorphon-based parametric models [@sanger2025parametric]. Vs30 maps to the NEHRP/ASCE-7 site class,
and because the proxy carries large residual scatter we propagate that as a **site-class probability**
rather than a point value (@sec-siteclass). Frequency sets depth: a Rayleigh wave of frequency $f$ is
most sensitive near $L \approx V_S/3f$, so a band of frequencies reads a profile of depths — the basis
of the depth kernels (`codameter`/`disba`) used by the $\delta v/v$ module. Bulk properties needed to
turn velocity into stress — density and the lithologic control on stiffness — come from the same SOLUS
static set (oven-dry bulk density, texture) and the geology/lithology layer that also feeds the storage
envelope, so the mechanical and hydrological grounds share one static description of the subsurface.
This stiffness field is the input to the Sanger liquefaction GLM [@sanger2025liquefaction] and, with
the water table, to the LandLab landslide chain (@sec-landlab).

### Petrophysical linking models {#sec-petro-grounds}

The two grounds meet in petrophysics: **water changes stiffness, and $\delta v/v$ measures the change** —
not as an empirical correlation but through an effective-medium relation with measurable material
parameters [@denolle2026unified]. The observable is a depth-averaged, frequency-dependent surface-wave
...
recovered through the Rayleigh kernels. What changes $V_S=\sqrt{\mu_{\rm eff}/\rho}$ is, to leading order,
the **effective shear modulus** $\mu_{\rm eff}$, not the density: a wetting-driven density change contributes
$\delta V_S/V_S = -\tfrac12\,\delta\rho/\rho$, two to three orders of magnitude smaller than the modulus term
for typical soils [@denolle2026unified]. The modulus is set by grain-contact mechanics — Hertz–Mindlin gives
$\mu_{\rm eff}\propto P_e^{1/3}$, so the pressure derivative $\mu'=d\mu/dP\sim\mu/P$ is very large at the low
confining pressures of the near surface; this is why seasonal $\delta v/v$ exceeds $0.1\%$ in soft sediments
but is below $0.01\%$ in crystalline rock, and why the signal lives in the shallowest layers. The two
hydrologic regimes, split at the water table, differ in *what sets the effective pressure* $P_e$.

**Saturated zone — pore pressure (poroelastic + acoustoelastic).** Below the water table the effective stress
is overburden minus pore pressure (Terzaghi/Biot [@biot1941general]); a head change $\Delta h$ raises pore
pressure by $\rho_w g\,\Delta h$, lowers effective stress, and softens the frame. In the poroelastic
surface-wave form the pore-pressure term $u^0$ reduces velocity,
$\left(\delta V_S/V_S\right)_{\rm Rayleigh} = -\tfrac{\mu'}{2\mu}u^0 + \tfrac{\mu'+1}{12\mu}T^0_{33}$,
and the underlying acoustoelastic (third-order-elastic) sensitivity $\delta v/v = \beta\,\epsilon_{kk}$ is
bridged to the seismological moduli by

$$
\beta = -\frac{\mu'\,\kappa}{2\mu}, \qquad \kappa_u = \frac{\kappa}{1-\alpha_B B},
$$

of order $\beta\approx-400$ for shallow sediments [@denolle2026unified; @clements2023seismic]. Whether the
*drained* modulus $\kappa$ or the *undrained* $\kappa_u$ (Biot coefficient $\alpha_B$, Skempton $B$) applies is
set by a Péclet number $\mathrm{Pe}=\omega L^2/c$ comparing the forcing rate to pore-pressure diffusion — so a
*rapid* rainfall pulse and a *slow* seasonal water-table change of equal magnitude produce different
$\delta v/v$ amplitudes. This is the mechanism by which the ambient field tracks groundwater
[@senss2006passive; @clements2018tracking]; our operational $\delta v/v = k_{sat}\,\Delta h$ is its
linearization, with $k_{sat}$ identified as $-\beta\,\rho_w g$ (a calibratable acoustoelastic coefficient), and
only a *relative* level is recovered.

**Vadose zone — partial saturation and dynamic capillarity.** Above the water table the effective confining
pressure carries a **capillary stress**, and $V_S=\sqrt{\mu_{\rm eff}/\rho}$ with $\mu_{\rm eff}$ set by
[@shi2026agroseismology; @denolle2026unified]

$$
P_e(z,t) = \sigma'_g(z,t) + P_c^{\rm eq}(S_w) + \tau(S_w)\,\frac{\partial S_w}{\partial t},
$$

the gravitational overburden $\sigma'_g$, the *equilibrium* capillary pressure $P_c^{\rm eq}(S_w)$ set by the
soil's water-retention curve, and a *rate-dependent dynamic* capillary stress $\tau(S_w)\,\partial_t S_w$
(Hassanizadeh–Gray non-equilibrium capillarity). Drying raises suction, raises $P_e$, and stiffens the frame;
wetting softens it. Shi et al. (2026) show three things that matter here: dynamic capillarity is *essential*
(a static-suction model underestimates the evaporation-driven velocity rise by ~50%); the dynamic coefficient
$\tau(S_w)$ differs between wetting and drying, so the $\delta v/v$–saturation relation is **hysteretic**
(path-dependent, not single-valued); and capillary suction can raise $|\beta|$ by factors of 2–5. Our
operational $\delta v/v = S_\theta\,\Delta\theta$ is therefore the *local linearization* of a hysteretic,
...
($\approx-1$ per unit $\theta$; the depth-diluted single-band value is $\approx-0.08$, @sec-classification)
applies to the *inverted* profile.

**From velocity change to stress — the interface.** Because both regimes act through
$V_S=\sqrt{\mu_{\rm eff}/\rho}$, the depth-separated $\delta V_S/V_S$ profile converts to a *physical stress*
change through the stress meter
...
with $\beta_{ij}$ built from those layered moduli and $\mu'(z)$ [@denolle2026unified]. This is what separates
the approach from a fitted correlation: the conversion of $\delta v/v$ to soil moisture and head is anchored to
**measurable material properties** — density $\rho$, bulk and shear moduli $\kappa,\mu$, the acoustoelastic
$\beta$, and the retention curve $P_c(S_w)$ — which become the borehole/lab **calibration targets** that carry
the MVP's constant $S_\theta$ and $k_{sat}$ toward a material-property-based, hysteresis-aware conversion. The
banded measurements are inverted through the Rayleigh kernels $\mathbf{G}$,
$\ \mathbf{d}=\mathbf{G}\,\mathbf{m}+\boldsymbol{\epsilon}$, with $\mathrm{cov}(\boldsymbol{\epsilon})$
marginalised over the processing ensemble ([`codameter`](https://github.com/Denolle-Lab/codameter)); splitting
the profile at the water table yields two hydrologic states from one measurement — a shallow **soil-moisture**
change (vadose, dynamic capillarity) and a deep **relative water-table** change (saturated, poroelastic) — each
assimilated into the matching hydrological state above.

## Data Grounds {#sec-data-grounds}

Every quantity above rests on data, and the data are heterogeneous in kind, resolution, and honesty.
Only two kinds are ever *assimilated* — **ground sensors** and **satellite retrievals** (observations);
everything else is a static prior or a physical/statistical model (@sec-classification). The three
kinds differ in a way that dictates the whole design.

**Ground sensors — dense in time, sparse in space, and not pristine.** USGS NWIS wells give real
depth-to-water but mix aquifers: a HAND-based shallow model must screen out deep confined-outwash
screens or it fits the wrong surface. SNOTEL and USCRN soil-moisture probes are point measurements at
a fixed sensor depth (typically 5 cm), with network gaps and a mountain/valley sampling bias. The UW
and CC seismic stations give continuous waveforms but $\delta v/v$ is a raw *processing* choice — coda
window, reference epoch, estimator — whose spread must be carried as data covariance, not hidden. The
common thread: raw sensor data are dense in time at a handful of points, so they anchor timing and
correct bias but cannot, alone, make a field.

**Satellite / gridded products — dense in space, sparse in time, at many scales.** These make the
field, but their **native resolution caps the resolution of anything derived from them**, and the
scales span three orders of magnitude: SMAP root-zone soil moisture at 9 km (36 km native L-band
footprint, sensing only the top $\sim$5 cm); MERRA-2 reanalysis at 0.5°; NLDAS at $\sim$12 km; GRACE
total-water-storage at $\sim$300 km; and the forcing climatologies TerraClimate ($\sim$4 km)
[@abatzoglou2018terraclimate] and PRISM (4 km, 800 m licensed). Treating a downscaled 9 km pixel as a
native 90 m observation — *footprint leakage* — is the central data hazard, so every downscaled field
records its native versus delivered resolution in provenance.

**Static high-resolution layers — the fine skeleton.** Time-invariant but sharp, these carry the 90 m
spatial texture that the coarse dynamic drivers lack: SOLUS100 soil (clay/sand/silt, pH, CEC, bulk
density, depth-to-lithic) at 100 m [@nauman2024solus], the USGS 3DEP DEM and its HAND/TWI/slope
derivatives at 10 m, the Vs30 field, and surficial geology/lithology. They set what the subsurface
*can* hold and how stiff it *can* be — the envelope that the dynamic observations modulate.

The consequence for the architecture is the static-envelope × dynamic-driver design used throughout:
fine static layers supply spatial structure, coarse observations supply the time signal, and the two
are reconciled at 90 m with an explicit, provenance-tracked uncertainty budget. The per-product access
paths, roles, and evaluation datasets are tabulated in @sec-twin and @sec-classification.
...
## Framework Description: the groundwater-level pipeline {#sec-framework}

This section details the pipeline for the **groundwater-level state** — the most mature of the three
coupled states, and the operational instantiation of the hydrological ground (@sec-hydro-grounds). The
stages below are the *statistical* backbone, **as designed**, for anchoring GWL to the well record: an
observation-anchored random-forest baseline, per-site β-maps for the climate response, and kriging of the
residuals. The closed pixel-wise **water budget** (@sec-hydro-grounds) then adds the *process* coupling — recharge,
capillary rise, and lateral flow — that ties GWL to the soil-moisture state (@sec-soilmoisture) and,
through the petrophysical bridge (@sec-petro-grounds), to the seismically-sensed stiffness state; the
three are assembled and animated together in the digital-twin demonstration (@sec-twin). The stage
figure below is therefore the GWL spine of a wider coupled system, not the whole framework.

@fig-pipeline shows the three-stage pipeline **as designed** — it is a diagram of the
intended architecture, not of the delivered real-data product. In the product reported
below (@sec-results), only the Stage-1 random-forest baseline is used as drawn: the entire
dynamic term comes from ordinary kriging of the *observed* per-well anomalies
(`gwl_dynamic_90m`), with no SPI-3, SWE, PDO or β term, because the Stage-2 β-map path has
not been fit or validated on the real well network (@sec-stage2). The key design principle
is that each stage is **physically motivated**, operates at a different timescale (climatological →
...
```{python}
#| label: fig-pipeline
#| fig-cap: "**Three-stage hybrid GWL framework, as designed.** This diagram is the *designed* pipeline, not the delivered real-data product. Observations (blue) anchor all three stages. Stage 1 (green) establishes the climatological depth-to-water baseline using an observation-anchored, HAND-based random-forest regression and kriging of residuals. Stage 2 (teal) is *designed* to capture interannual and seasonal variability through per-site OLS β-maps; that path has not been fit or validated on the real well network, and only three β terms are ever fitted (SPI-3, SWE, PDO) — the atmospheric-river term β₄ is a placeholder fixed at zero. Stage 3 (purple) kriging is designed to close the gap between the climate reconstruction and the well record. In the delivered product the whole dynamic term instead comes from ordinary kriging of the *observed* per-well anomalies (`gwl_dynamic_90m`, `src/models/gwl_dynamic.py`), and the uncertainty stack shipped with it carries three terms with no response term σ_resp, since no Stage-2 OLS fit was run on the real wells. GAIA ecosystem data (orange dashed) flows into Stage 1 and Stage 2."
#| fig-width: 11
#| fig-height: 6
...
**What is actually operational today.** The β-map path above is implemented — per-site
OLS, ordinary kriging of the fitted β coefficients to the 90 m grid, and the matrix
multiply that reconstructs ΔDTW all live in `src/models/climate_response.py`
(`:103-137`, `:140-176`, `:374-378`), and this is the pipeline's declared Stage 2
(`Makefile:172`) — but it has **not been fit or validated on the real well network**.
(The atmospheric-river β₄ term is separately a placeholder fixed at zero — see the
caption of @fig-pnw-signals.) The Stage-2 illustration in
@fig-climate-response is a *synthetic, hypothetical* well, and no fitted β maps accompany
the real-data product shown in this report. The time-varying signal in that product
(@sec-results, @fig-gwl-anim) is instead produced by `gwl_dynamic_90m`
(`src/models/gwl_dynamic.py`): the well record is first restricted to the analysis window
(2010-04–2013-03), then each well's mean *over that window* is removed — so the anomaly is
a departure from the analysis-window mean, not from a long-term or climatological mean —
and the resulting *observed* monthly anomaly is ordinary-kriged, downscaled to 90 m, and
added to the Stage-1 baseline. That path contains no SPI-3, SWE, PDO, or β term
whatsoever. Scenario generation by matrix multiply is therefore, at the time of writing, a
design capability of the β-map path — not a demonstrated property of the product reported
here.
...
- **A downscaling module is required regardless.** ParFlow-CONUS is ~1 km; our target is
  ...
  flow is handled in assimilation (above). Rung 2 itself is **not implemented**: no TFN /
  impulse-response model exists in the codebase, and it remains future work. Rung 1 (the
  β-map) *is* implemented and is the pipeline's declared Stage 2, but has not yet been fit
  on the real well network; what is operational on real data today is the kriged
  observed-anomaly Stage-2/3 signal described in @sec-results.

### Stage 3: Kriging of observation residuals {#sec-stage3}

Stage 3 is designed to remove any systematic bias between the climate reconstruction and
the actual well observations. As with Stage 2, what follows describes the designed
pipeline (`src/models/interpolate_residuals.py`), not the assembly used for the real-data
product reported in @sec-results — see **What is actually operational today** in
@sec-stage2. For each month $t$:
...
handle non-Gaussian residual distributions. In the designed Stage-2/3 assembly the final
product is:
...
This equation states the *designed* assembly, and it is what
`src/models/interpolate_residuals.py` computes; it is **not** how the real-data product
shown in this report was assembled. Because no β maps have been fit to the real well
network, there is no climate reconstruction to take residuals against, and the delivered
90 m monthly product instead adds a single kriged *observed*-anomaly term to the Stage-1
baseline (`gwl_dynamic_90m`; @sec-stage2, @sec-results) — i.e. its Stage-2 and Stage-3
terms are not separately identified. Readers arriving here directly should read this
section as specification, not as a description of the numbers reported in @sec-results.
...
4. **Atmospheric rivers (impulse)**: individual AR events deliver 20–40% of annual
   ...
   integration with Stage IV radar-gauge data will quantify it once a precipitation-product intercomparison is complete.
...
This four-term budget describes the **designed** full β-map pipeline. The budget actually
computed for the delivered real-data product has **three** terms — static (random-forest
baseline), dynamic kriging, and downscaling representativeness (the three-term budget in
"One downscaling and uncertainty budget for both variables", @sec-soilmoisture, and in
`data/processed/provenance.json`) — with no $\sigma_{\text{response}}$ term, because no
Stage-2 OLS fit was run on the real well network to produce one.
...
#| fig-cap: "**Comparison of GWL modeling paradigms.** Each approach is characterized by its data requirements, spatial resolution, temporal capability, and computational cost. The hybrid framework proposed here occupies the middle ground: it is more physically grounded than pure statistical interpolation (Fan et al. 2013, HydroGEN) but far less demanding than full numerical simulation. The key differentiators relative to prior hybrid work are the use of HAND (vs. raw DEM elevation) and the β-map framework for temporal extrapolation. The second of these is a *designed* capability: the β-map temporal-extrapolation path is implemented in code but has not yet been fit or validated on the real well network, and the temporal capability actually demonstrated in this report is ordinary kriging of observed well anomalies — so the position plotted for \"This work\" is the designed target, not a validated result."
#| fig-width: 11
#| fig-height: 5.5
...
| Temporal model | None (static) | None (static) | SPI regression | **Kriged observed anomalies** (demonstrated); **β-map (SPI+SWE+PDO)** (implemented, not yet fit on real wells) |
...
| Scenario generation | N/A | N/A | Re-run regression | **Matrix multiply** (design capability; not yet demonstrated) |

The **This work** column deliberately mixes demonstrated and designed capability. The
HAND predictor, spatial block cross-validation, absence of tiling artifacts, and DataTree +
STAC output describe the real-data product; the β-map temporal model and matrix-multiply
scenario generation are implemented in code but have not yet been fit or validated on the
real well network, and the temporal term that is actually demonstrated here is ordinary
kriging of observed well anomalies (@sec-results).
...
| Soil moisture | SOLUS100 → Saxton–Rawls envelope (90 m) | precipitation $P$ & PET → Thornthwaite–Mather bucket | implemented |
...
### Soil-moisture dynamic driver — precipitation forcing and an honest evaluation
...
$\mathrm{AWC} = (\theta_{fc}-\theta_{wp})\,z_r$ (root depth $z_r = 1$ m) is forced by monthly
**precipitation** $P$ and reference evapotranspiration PET. The precipitation source is
deliberately not fixed — it is a swappable input (station-based PRISM, a gridded reanalysis, or a
downscaled forecast); the MVP shown here happens to use TerraClimate [@abatzoglou2018terraclimate]:
...
Two roles must be kept distinct. The **precipitation and reference ET are forcing inputs** to our
water balance. A separate **soil-water-storage product** (here TerraClimate's) is used only as a
**evaluation dataset** — it never enters the estimator. Because, in this MVP, both the forcing and that
evaluation dataset field derive from a Thornthwaite–Mather balance driven by the *same* $P$ and PET, the
$r = 0.94$ agreement between our $\theta$ and the TerraClimate soil field (domain mean; per-cell median $0.93$, for the shipped
...
MERRA-2 is a model evaluation dataset rather than an independent satellite retrieval, but its *root-zone*
...
the precipitation-forced wetness, ~2 km for the kriged GWL anomaly — and bilinearly
...
also making the TerraClimate evaluation more independent. Second, **downscaling is modular**:
...
- **Stage 2 signs (planned):** these are *acceptance criteria for a fit that has not yet
  been run*, not diagnostics that have passed — no β maps have been fit to the real well
  network (@sec-stage2). When that fit is performed we will check that the median per-well
  R² exceeds the HydroGEN analogue; that the AIC-selected SWE lag > 0 in mountain-front
  wells; and that β₁ (SPI-3) is negative everywhere (wetter → shallower). A significance
  criterion on the PDO term [@kuss2014multi] is *not* computable from the current
  implementation, which reports R², residual σ, AIC, and $n$ but no coefficient $p$-values;
  applying it requires first extending the per-site fit to report them.
...
over a densely-instrumented window. This time-varying field is produced by ordinary-kriging the
*observed* per-well monthly anomaly and adding it to the Stage-1 baseline (`gwl_dynamic_90m`);
it is not a β-map reconstruction from climate indices.
...
capacity, porosity, available water); a **total-water bucket** forced by precipitation fills it
...
rainfall), and the same operators run on measured dv/v once the multi-year waveform run lands. The
shallow (vadose) band now carries a several-percent seasonal signal — peak-to-peak $\approx 9\%$
during strong wetting and drying — so after depth inversion the near-surface stiffness moves
visibly: the domain-mean Vs30 spans $\approx 425$–$439$ m s⁻¹ over the year ($\approx 3\%$), and
near constraining stations the swing is larger still. Because Vs30 is a 30 m time-*average* and the
point assimilation reverts to the static prior away from sensors, that domain-mean range is a lower
bound on the local response, not the full shallow amplitude — the honest consequence of measuring a
30 m average with a locally-constrained analysis.
...
### Vs30 uncertainty as site-class probability {#sec-siteclass}

A geotechnical consumer does not act on a raw "15 % ± " on Vs30 — it acts on the **NEHRP/ASCE 7
site class** and on the risk of crossing a design boundary (180 / 360 / 760 m s⁻¹). We therefore
report the Vs30 uncertainty as a **site-class probability**: given the per-cell Vs30 mean and its
propagated 1σ, `fetch_vs30.nehrp_class_probabilities` integrates a Gaussian (or lognormal) over the
class boundaries to return $P(\text{E}, \text{D}, \text{C}, \text{B}, \text{A})$, and by differencing
the CDF, the probability of crossing any single boundary. A cell that sits near a boundary honestly
splits its mass across two classes rather than being forced into one.

```{python}
#| output: asis
import os, sys
for _p in (os.getcwd(), os.path.abspath("..")):        # kernel cwd is docs/; repo root holds src/
    if os.path.isdir(os.path.join(_p, "src")) and _p not in sys.path:
        sys.path.insert(0, _p)
from src.data.fetch_vs30 import nehrp_class_probabilities, NEHRP_CLASSES
examples = [("soft valley fill", 200.0, 30.0),
            ("firm / stiff soil", 350.0, 40.0),
            ("weathered rock", 620.0, 90.0)]
print("| Representative cell | Vs30 (m/s) | 1σ | " + " | ".join(f"P({c})" for c in NEHRP_CLASSES) + " |")
print("|---|---|---|" + "---|" * 5)
for name, mu, sd in examples:
    cells = " | ".join(f"{x:.2f}" for x in nehrp_class_probabilities(mu, sd))
    print(f"| {name} | {mu:.0f} | {sd:.0f} | {cells} |")
```

The middle row makes the point: a 350 m s⁻¹ firm-soil cell 40 m s⁻¹ from the D|C boundary carries a
real probability of being class C, and that split — not a symmetric percent bar — is what a
site-response or liquefaction screen should propagate.
...
| USGS 3DEP DEM | 3DEP (`py3dep`) -> HAND/TWI/slope | 10 m -> 90 m | terrain prior (HAND, TWI); slope feeds the Vs30 row below |
...
| NLDAS-2 soil moisture | NASA GES DISC (`earthaccess`) | ~12 km | soil moisture (gridded evaluation dataset) |
...
- **Evaluation dataset (model / reanalysis).** Model products (TerraClimate, PRISM, MERRA-2, NLDAS) used as
...
| Soil-moisture validation | MERRA-2 (model evaluation dataset); SNOTEL (real); SMAP (real, staged) | n/a | SMAP L3/L4, NLDAS, lowland USCRN / ISMN |
...
![Coupled water budget on real domain-mean precipitation forcing and the real 90 m TWI field. Left:
...
| Soil moisture | domain-mean $r$; bias; RMSE | 0.85; −0.08; 0.095 | MERRA-2 root zone (incl. 2025) | **MODEL (evaluation)** |
...
independent skill. MERRA-2 is a model evaluation dataset (shared forcing family). The dv/v recovery is still
...
## Downstream: LandLab landslide coupling {#sec-landlab}

The complementary downstream consumer is the **LandLab** landslide study — the geotechnical
data-prep pipeline `gaia-hazlab/landslide-data-prep`, which drives LandLab's infinite-slope
`LandslideProbability` component (the Monte-Carlo Factor of Safety, $\Pr(FS \le 1)$). Pore pressure
from a shallow, storm-driven water table is the proximate trigger of the shallow landslides it
models, so this reanalysis is the natural supplier of that hydrologic state.

### Division of labour

The interface is clean because the two repositories own different halves of the input stack:

- **`landslide-data-prep` owns the static stack** — DEM (3DEP), SOLUS100 soil, and NLCD land cover
  turned into cohesion, internal friction angle, and leaf-area index. It harmonises these onto the
  LandLab analysis grid. We do **not** duplicate or re-export them.
- **This reanalysis owns the dynamic / hydrological state** and exports *only* those fields, on the
  **same grid** so they register cell-for-cell onto the static stack.

The seam between the two is a small set of parameters that are *static* (so nominally the data-prep
side's job) but are *also* structural inputs to our water budget — chiefly the saturated hydraulic
conductivity $K_{sat}$ and soil thickness. For our recharge/drainage physics and LandLab's
transmissivity to be consistent, both sides must derive $K_{sat}$ the **same way**; we make that
choice explicit and swappable (below) rather than hard-coding one pedotransfer.

### The dynamic export contract

We export the following via `src.io.landlab_export`, in the DataHub canonical vocabulary, as
LandLab-ready ESRI ASCII (`.asc`) plus a COG and a provenance manifest:

| Canonical field | Meaning | Units | Cadence | Uncertainty |
|---|---|---|---|---|
| `water_table__depth` | depth to water table; **mean and seasonal-high state** | m | monthly + wet-season high | per-cell σ + confidence mask |
| `soil_moisture__saturation_fraction` | $S = \theta / n$ | – | monthly | from θ envelope + driver |
| `soil_water__recharge_rate` | groundwater recharge (water budget) | mm day⁻¹ | monthly (mean/max) | σ ≈ 0.1·mean |

The landslide-relevant state is the **wet-season high** table (small depth-to-water), not the annual
mean: when a monthly depth-to-water series is supplied (`--dtw-series`), `seasonal_high_water_table`
also exports its shallow-tail quantile. The operational monthly GWL series is still in progress — only
a short pilot exists today — so the default run writes the mean, and the seasonal-high path is wired
and ready rather than populated. Every exported field carries its per-cell 1σ; the **water table** is
additionally blanked by the variogram-driven well-density confidence mask (@sec-validation) so
unsupported upland cells (fractured hillslopes — precisely the sparse-well regime where this framework
is weakest) export as no-data rather than a false mean. The saturation-fraction and recharge fields are
model quantities: they carry σ but not the well-density mask, which is specific to the groundwater state.

The command `python -m src.io.landlab_export` (pixi task `landlab-export`) assembles this bundle from
the real products: the water table from the baseline depth-to-water with its RF ⊕ kriging σ and the
well-density confidence mask, the saturation fraction from the soil-moisture θ over the porosity, and
recharge from the coupled water budget run over the precipitation forcing (@fig-landlab-export).

![LandLab dynamic export: the three canonical hydrological fields (top) and their per-cell 1σ
(bottom), on the 90 m grid, the water table masked to the well-supported domain. The water table
(HAND baseline) and the saturation fraction both carry fine 90 m structure — the saturation is
downscaled the same way as the rest of the product and the digital twin (coarse relative wetness
re-expressed through the fine 90 m soil envelope, $\theta = \theta_{wp} + sf\,(\theta_{sat}-\theta_{wp})$),
so its 90 m detail is the static texture while its temporal signal stays at the driver scale.
Recharge is exported at the coarse ~4 km forcing scale (a P/PET-dominated flux, so its
information is genuinely coarse; visible as blocky cells). Each field's spatial scale is recorded in
the manifest so a resampled cell is not read as a native observation.](assets/landlab_export.png){#fig-landlab-export}

### Shared static soil-hydromechanical set

We ingest the *same* SOLUS100 static set the data-prep side uses — texture (clay/sand/silt), pH, CEC,
oven-dry bulk density, and depth-to-lithic (`anylithicdpt` → soil thickness) — via
`fetch_public.fetch_solus`, so our soil state is built on identical inputs. These feed both the
Saxton–Rawls retention envelope (θ_wp/θ_fc/θ_sat) that sizes our bucket and the $K_{sat}$ choice below.

### Modular $K_{sat}$ and transmissivity {#sec-ksat}

There is no single correct pedotransfer for $K_{sat}$, and the choice materially changes both our
drainage flux and LandLab's transmissivity $T$. `src.models.soil_hydraulics` therefore exposes
$K_{sat}$ and $T$ as a **registry of methods** that a user selects; the default is Saxton–Rawls, but
selecting the LandLab pedotransfer makes our hydrology and the Factor-of-Safety mutually consistent.
Output units are canonical: $K_{sat}$ in m day⁻¹, $T$ in m² day⁻¹.

$K_{sat}$ methods (`saturated_conductivity(method=…)`):

- **`saxton_rawls`** (default) — Saxton & Rawls (2006) [@saxton2006soil] from sand%, clay%, and
  organic-matter%; also yields the θ_wp/θ_fc/θ_sat envelope. Native mm hr⁻¹ converted to m day⁻¹.
- **`solus_pedotransfer`** — the log-linear PTF used by `landslide-data-prep`,
  $\log_{10} K_{sat} = 0.8478 + 0.2612\,\mathrm{pH} - 0.0233\,\mathrm{clay} - 0.0127\,\mathrm{silt}
  - 0.0104\,\mathrm{CEC}$ (clay/silt %, CEC in cmol(+) kg⁻¹). **Select this for LandLab consistency.**
  Its native output unit is undocumented at source, so a `unit_scale` hook converts it to m day⁻¹
  once confirmed with the data-prep authors — flagged rather than silently assumed.
- **`provided`** — pass through a measured or data-product $K_{sat}$ field (e.g. POLARIS Ksat).

Transmissivity methods (`transmissivity(ksat, thickness, method=…)`):

- **`ksat_x_thickness`** — $T = K_{sat}\,h$ (the `landslide-data-prep` convention).
- **`ksat_x_thickness_anisotropy`** — $T = K_{sat}\,h\,f$ with a lateral:vertical anisotropy factor
  $f$ (DataHub canonical default $f=2.5$).
- **`topmodel_exponential`** — $T = K_{sat}\,d$, the effective profile transmissivity of an
  exponential store with decay depth $d = 1/f$ (TOPMODEL-style), for hillslopes where conductivity
  decays sharply with depth.

The registry is deliberately open: a new PTF (Cosby, Rosetta, …) is one function plus a dictionary
entry, and the report/manifest records which method produced a given field so downstream users can
reproduce or re-select it.

### Grid parity and the resolution caveat

Our native grid is 90 m EPSG:5070; the LandLab AOI grid is finer (often 10 m). `align_to_grid`
reprojects each dynamic field onto the caller's template grid so it registers with the static stack,
and the export manifest records **native vs export resolution** for every field. Reprojection to a
finer grid *resamples* — it does not add information — so a downscaled 90 m (or coarser satellite)
cell is never to be read as a native fine-scale observation (the DataHub "footprint-leakage"
caution). A physics-based downscaler for this step is future work (@sec-roadmap).
...
evaluation dataset cannot supply. A **denser Cascade soil-moisture network** is what lets the anchor
...
On the downstream side, the **LandLab handoff is now live** (@sec-landlab): `landlab_export` writes the
dynamic hydrological state (`water_table__depth`, `soil_moisture__saturation_fraction`, recharge) as
canonical LandLab-ready fields on the shared grid, with the static soil-hydromechanical parameters
reconciled through the modular $K_{sat}$ registry. The next major step is **forecast forcing**: driving
the same water budget with observed rainfall (PRISM, the preferred source) for hindcasts and with
AI weather forecasts (GraphCast / ACE2, precipitation downscaled to the analysis grid) for a forward
antecedent-state ensemble, cross-validated against an independent gridded product (e.g. ERA5-Land).
Together these carry the pilot from a demonstration to a calibrated, observation-anchored,
uncertainty-aware coupled reanalysis that feeds the liquefaction and landslide hazard chains.
...
AR catalog exists for the PNW at 4 km resolution. A Stage IV radar-gauge precipitation intercomparison will deliver this. Once AR counts are
...
2. **Captures PNW hydroclimatic variability** in the delivered 90 m monthly product by
   ordinary-kriging the observed per-well anomalies onto the static baseline, and supplies
   — implemented in the pipeline, but not yet fit or validated on the real well network —
   a per-site β-coefficient decomposition of those anomalies into SPI-3 (seasonal
   precipitation), SWE (snowmelt recharge), and PDO (ENSO/decadal) components, consistent
   with the established literature on western US groundwater teleconnections
   [@bloomfield2013analysis; @kuss2014multi].

3. **Is designed to enable forward GWL scenarios** from any climate projection via a
   matrix multiply of β maps and climate indices, without re-running kriging — which
   would make the framework compatible with Earth2Studio ensemble outputs. This is stated
   as capability, not as result: the β-map code exists and is the declared Stage 2, but no
   β maps have been fit to the real well network, and the transfer-function-noise model of
   Rung 2 is not implemented. Demonstrating and validating both on real wells is the
   immediate next step.
...
5. **Generalizes to a coupled soil-reanalysis** (@sec-soilmoisture): the same
   ...
   companion 90 m **soil-moisture** state variable (SOLUS→Saxton–Rawls envelope forced by a precipitation-driven Thornthwaite–Mather balance), with a shared, explicitly
...
(SMAP, in-situ), the current evaluation against TerraClimate being a consistency test
...
The `make` targets above build the **designed** pipeline (Stage 1 → β-map Stage 2 →
residual Stage 3). They do **not** reproduce the real-data 90 m monthly product reported in
@sec-results: that product — @fig-gwl-anim and the uncertainty budget in
`data/processed/provenance.json` — is produced by `gwl_dynamic_90m`, which has no Makefile
target and is run via `pixi run python old/notebooks/make_products_90m.py` once the
upstream targets it reads have been built (`make baseline` for the Stage-1 rasters, plus
the QC'd monthly well parquet and the soil/climate inputs).
...
| `src/models/climate_response.py` | Stage 2 per-site OLS β-maps (designed path; not yet fit on real wells) |
| `src/models/interpolate_residuals.py` | Stage 3 kriging + final assembly (designed path) |
| `src/models/gwl_dynamic.py` | Kriged *observed*-anomaly dynamic DTW (`gwl_dynamic_90m`) — the path that actually produced @fig-gwl-anim and `data/processed/provenance.json`. Invoked from `old/notebooks/make_products_90m.py`, **not** from any Makefile target |
