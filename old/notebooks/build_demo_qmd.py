...
({snotel['n_station_months']} station-months) — far below the shared-forcing r≈{r_dm:.2f}, exactly as
...
and per-station r (snow off→on). The independent r sits far below the shared-forcing r≈{r_dm:.2f} — as
...
**Vs30 is ~100% terrain (HAND)**, groundwater ~61% HAND + ~26% clay, soil moisture ~89% sand.
**Caveat.** Those field-level shares largely restate how the twin was built. The θ panel is the
static Saxton-Rawls θ_fc envelope (a closed form in sand/clay) plus the near-station dv/v increment,
so ~89% sand is self-recovery, not a finding; the groundwater share is inherited from a baseline RF
fit to real wells, reflecting that fitted mapping, not an independent finding. The ~100% HAND share
holds only under the synthetic HAND-ramp fallback, where Vs30 is *defined* from HAND and is likewise
self-recovery; but the twin now records vs30_90m.tif (Wald-Allen slope proxy) as its Vs30 source,
under which that attribution fails, so the entire quoted Vs30 row — the HAND share and the
small-dv/v claim alike — is stale and is being reconciled separately. These become independent
findings only once the covariate relations are learned from observed fields rather than imposed.
dv/v is a small field-level term in this MVP *by construction* — it only perturbs the state near the
seismic stations. **Update level** — the precision-weighted assimilation share shows which sensor
sets the near-station correction: ~94% wells (GWL), ~95% seismic dv/v (soil moisture), ~99% seismic
(Vs30). The first is a field-wide accounting of the fields *as constructed* (see caveat); the second
only the near-station refinement.

![Attribution: RF feature importance per state (top; covariates dominate, dv/v minor at the field
level — largely by construction, see caveat) and the sensor share of the near-station update
(bottom).](../figures/demo/digital_twin_attribution.png)
